...
resource "aws_lambda_function" "get_board" {
  role             = aws_iam_role.lambda.arn
  handler          = "handlers.get_board"
  runtime          = "python3.6"
  filename         = var.file
...
resource "aws_api_gateway_deployment" "test" {
  depends_on = [
    ...
    aws_api_gateway_integration.GET__board,
...
resource "aws_api_gateway_method" "GET__board" {
  rest_api_id   = aws_api_gateway_rest_api.postboard.id
...
resource "aws_api_gateway_integration" "GET__board" {
  rest_api_id = aws_api_gateway_rest_api.postboard.id
  resource_id = aws_api_gateway_method.GET__board.resource_id
  http_method = aws_api_gateway_method.GET__board.http_method

  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.get_board.invoke_arn
}

resource "aws_lambda_permission" "apigw_invoke_get_board" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.get_board.function_name
  principal     = "apigateway.amazonaws.com"
